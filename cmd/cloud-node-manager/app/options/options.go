...
func (o *CloudNodeManagerOptions) Flags() cliflag.NamedFlagSets {
	...
	fs.BoolVar(&o.EnableNodeEventChecker, "enable-event-checker", true, "Should enable the NodeEventChecker to check for Azure scheduled events. Can only be set to true if --use-instance-metadata is also true. If false, the NodeEventChecker will not run and no events will be recorded in the node status.")
